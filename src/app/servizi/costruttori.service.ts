...
export class CostruttoriService {
   ...
   */
  async getTuttiCostruttori(): Promise<Costruttore[]> {
    if (!this.cache.costBool)
      await this.loadTuttiCostruttoriRicorsivo(0);

    return Array.from(this.cache.costruttori.values());
  }
  /**
   ...
   */
  private async loadTuttiCostruttoriRicorsivo(offset: number) {
    //Richiesta API per ottenere i costruttori di F1
    const costruttori: any = await lastValueFrom(this.api.getDataF1Api('https://ergast.com/api/f1/constructors.json', offset));
    console.log(costruttori);

    //Richiesta API wiki per ottenere le immagini di tutti i circuiti
    const wikiData: any = await lastValueFrom(this.api.getDataWikipedia(this.api.estraiTitoliDaVettoreGenerico(costruttori.MRData.ConstructorTable.Constructors), this.api.imageSize));
    ...
    if (this.cache.costruttori.size < costruttori.MRData.total) {
      this.loadTuttiCostruttoriRicorsivo(offset + costruttori.MRData.ConstructorTable.Constructors.length);
    } else if (this.cache.costruttori.size == costruttori.MRData.total) {
  ...
  async getPilotiCostruttore(id: string) {
    ...
    if (!costruttore.pilotiBool)
      await this.loadPilotiCostruttoreRicorsivo(costruttore, 0);
   ...
   */
  private async loadPilotiCostruttoreRicorsivo(costruttore: Costruttore, offset: number) {
    if (!costruttore.pilotiBool) {
      //Richiesta API per ottenere i piloti
      const piloti: any = await lastValueFrom(this.api.getDataF1Api(`https://ergast.com/api/f1/constructors/${costruttore.id}/drivers.json`, offset));
      console.log(piloti);
   ...
   */
  async getStagioniERisultatoCostruttore(id: string): Promise<PostoClassificaCostruttori[]> {
    //Prendo il costruttore
    const costruttore: Costruttore | undefined = await this.getCostruttore(id);
    if (costruttore === undefined)
    ...
    if (!costruttore.stagioniBool)
      await this.loadStagioniERisultatoCostruttoreicorsivo(costruttore, 0);
  ...
  private async loadStagioniERisultatoCostruttoreicorsivo(costruttore: Costruttore, offset: number) {
    if (!costruttore.stagioniBool) {
      //Richiesta API per ottenere le stagioni
      const stagioniEPosizione: any = await lastValueFrom(this.api.getDataF1Api(`https://ergast.com/api/f1/constructors/${costruttore.id}/constructorStandings.json`, offset));
      ...
      stagioniEPosizione.MRData.StandingsTable.StandingsLists.forEach(async (posizione: any) => {
        ...
        //Prendo la posizione
        let postoStagione: PostoClassificaCostruttori | undefined = stagione.classificaCostruttori.get(posizione.ConstructorStandings[0].position);
        //Se non è in cache lo aggiungo
        if (postoStagione === undefined) {
          //Creazione della posizione
          postoStagione = new PostoClassificaCostruttori(stagione, posizione.ConstructorStandings[0].position, posizione.ConstructorStandings[0].points,
            posizione.ConstructorStandings[0].wins, costruttore);
